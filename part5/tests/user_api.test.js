...
describe('having initially one user in db', () => {
  ...
  test('server responds with 400 for short username', async () => {
    ...
    await api
      .post('/api/users')
      .send(badUser)
      .expect(400, {error: 'User validation failed: username: Path `username` (`sl`) is shorter than the minimum allowed length (3).'})
  })
  ...
  test('server responds with 400 for repeated username', async () => {
    ...
    await api
      .post('/api/users')
      .send(badUser)
      .expect(400, {error: 'User validation failed: username: Error, expected `username` to be unique. Value: `root`'})
  })
